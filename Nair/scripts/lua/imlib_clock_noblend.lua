...
image_path = os.getenv('HOME')..'/.config/conky/Nair/images/'
...
function create_clock(theme, arc_s, arc_m, arc_h)
    ...
    -- Set blending mode to false to keep original color
    imlib_context_set_blend(0) -- Disable blending mode for the clock face

    -- Load and rotate clock hands
    ...
    -- Draw clock face without blending to retain original color
    imlib_blend_image_onto_image(imgFace, 1, 0, 0, w_img, h_img, 0, 0, w_img, h_img)

    -- Re-enable blending mode for hands
    imlib_context_set_blend(1)

    -- Draw clock hands centered on the face
    imlib_blend_image_onto_image(imgHR, 1, 0, 0, w_imgH, h_imgH, (w_img - w_imgH) / 2, (h_img - h_imgH) / 2, w_imgH, h_imgH)
    imlib_blend_image_onto_image(imgMR, 1, 0, 0, w_imgM, h_imgM, (w_img - w_imgM) / 2, (h_img - h_imgM) / 2, w_imgM, h_imgM)
    imlib_blend_image_onto_image(imgSR, 1, 0, 0, w_imgS, h_imgS, (w_img - w_imgS) / 2, (h_img - h_imgS) / 2, w_imgS, h_imgS)

    -- Free images to prevent memory leaks
    fFreeImage(imgFace)
    fFreeImage(imgH)
    fFreeImage(imgHR)
    fFreeImage(imgM)
